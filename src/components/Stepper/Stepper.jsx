...
const Stepper = () => {
	...
	const handleInteresChange = (value) => {
		setFormValues((prevState) => ({
			...prevState,
			interest: prevState.interest.includes(value) ? prevState.interest.filter((interest) => interest !== value) : [...prevState.interest, value],
		}));
	};
	...
	const handleAllergenChange = (value) => {
		setFormValues((prevState) => ({
			...prevState,
			allergen: prevState.allergen.includes(value) ? prevState.allergen.filter((allergen) => allergen !== value) : [...prevState.allergen, value],
		}));
	};

	const handleCheckboxChange = (group, value) => {
		setFormValues((prevValues) => {
			const newValues = prevValues[group].includes(value) ? prevValues[group].filter((i) => i !== value) : [...prevValues[group], value];
			return {
				...prevValues,
				[group]: newValues,
	...
	const handleNext = () => {
		...
		switch (activeStep) {
			...
			case 3:
				...
				break;
			case 4:
                isValid = formValues.allergen.length >= 0;
				break;
	...
	return (
		<>
			...
			<Container maxW='container.md' p={4} display={activeStep === 0 ? 'block' : 'none'}>
				<Box>
					<div className='container_login'>
						...
						<button className='btn_login_register' onClick={handleLoginClick}>
							Iniciar Sesión
						</button>
					</div>
					<div className='spam'>
					...
					<FormControl isRequired>
						...
						<Input type='email' name='email' value={formValues.email} onChange={handleChange} placeholder='ejemplo@gmail.com' />
			...
			<Container maxW='container.md' p={4} display={activeStep === 1 ? 'block' : 'none'}>
				<Box>
					...
					<FormControl isRequired mt={4}>
						...
						<Input name='nombre' value={formValues.nombre} onChange={handleChange} placeholder='Ejemplo' />
					</FormControl>

					<FormControl isRequired mt={4}>
						<FormLabel>Apellidos</FormLabel>
						<Input name='apellido' value={formValues.apellido} onChange={handleChange} placeholder='Ejemplo' />
					</FormControl>

					<FormControl isRequired mt={4}>
						<FormLabel>País</FormLabel>
						<div className='container_work'>
							<select className='select_work' name='pais' value={formValues.pais} onChange={handleChange}>
								<option value=''></option>
					...
					<FormControl isRequired mt={4}>
						...
						<Input name='direccion' value={formValues.direccion} onChange={handleChange} placeholder='Ejemplo' />
					</FormControl>

					<FormControl isRequired mt={4}>
						<FormLabel>Código postal / ZIP</FormLabel>
						<Input name='codigo_postal' value={formValues.codigo_postal} onChange={handleChange} placeholder='Ejemplo' />
					</FormControl>

					<FormControl isRequired mt={4}>
						<FormLabel>Población</FormLabel>
						<Input name='poblacion' value={formValues.poblacion} onChange={handleChange} placeholder='Ejemplo' />
					</FormControl>

					<FormControl isRequired mt={4}>
						<FormLabel>Provincia</FormLabel>
						<Input name='provincia' value={formValues.provincia} onChange={handleChange} placeholder='Ejemplo' />
					</FormControl>

					<FormControl isRequired mt={4}>
						<FormLabel>Teléfono</FormLabel>
						<Input name='telefono' value={formValues.telefono} onChange={handleChange} placeholder='Ejemplo' />
					</FormControl>

					<FormControl isRequired mt={4}>
						<FormLabel>Añadir foto</FormLabel>
						<InputGroup>
							<Input name='image_path' value={formValues.image_path} onChange={handleChange} placeholder='' height='80px' />
							<Box position='absolute' top='50%' left='50%' transform='translate(-50%, -50%)' pointerEvents='none' color='gray.400'>
								<LuImagePlus />
							</Box>
						</InputGroup>
					</FormControl>
			...
			<Container maxW='container.md' p={4} display={activeStep === 2 ? 'block' : 'none'}>
				<Box>
					...
					<FormControl isRequired mt={4}>
						...
						<Input name='nombre_empresa' value={formValues.company} onChange={handleChange} placeholder='Tu empresa' />
					</FormControl>

					<FormControl isRequired mt={4}>
						<FormLabel>Puesto de trabajo</FormLabel>
						<div className='container_work'>
							<select className='select_work' name='puesto_trabajo' value={formValues.puesto_trabajo} onChange={handleChange}>
								<option value=''></option>
					...
					<FormControl isRequired mt={4}>
						...
						<Input name='linkedin' value={formValues.linkedin} onChange={handleChange} placeholder='Ejemplo' />
					</FormControl>
				</Box>
			</Container>
			...
			<Container maxW='container.md' p={4} display={activeStep === 5 ? 'block' : 'none'}>
				<Box>
					{activeStep == 4 || activeStep == 5 && <Products />}
				</Box>
			</Container>
			...
			<Box className='btn_container' mt={4} display='flex' justifyContent='flex-center'>
				<Button
					...
					isLoading={activeStep === steps.length - 1 && isSubmitting}
					borderRadius='80px'
				>
